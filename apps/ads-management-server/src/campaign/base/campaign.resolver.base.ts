...
export class CampaignResolverBase {
  ...
  @graphql.ResolveField(() => [AdGroup], { name: "adGroups" })
  async findAdGroups(
    @graphql.Parent() parent: Campaign,
    @graphql.Args() args: AdGroupFindManyArgs
  ): Promise<AdGroup[]> {
    const results = await this.service.findAdGroups(parent.id, args);
  ...
  async CampaignFreeze(
    ...
    return this.service.CampaignFreeze(args);
  }

  @graphql.Mutation(() => CampaignCountArgs)
  async FreezeCampaign(
    @graphql.Args()
    args: CampaignWhereUniqueInput
  ): Promise<CampaignCountArgs> {
    return this.service.FreezeCampaign(args);
  }
}
